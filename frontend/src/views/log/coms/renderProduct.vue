...
export default {
  ...
  render: function(h) {
    ...
    let isChanged = beforeData
      ? beforeData[`name${this.LANGUAGE}`] &&
        beforeData[`name${this.LANGUAGE}`] !== data[`name${this.LANGUAGE}`]
      : false;
    const name = data.code ? data[`name${this.LANGUAGE}`] : "--";
    const desc = data.code ? data.desc : "--";
    ...
    let vdom = h("div", { style: { position: "relative" } }, result);
    return vdom;
  },
  props: {
    firstData: {
