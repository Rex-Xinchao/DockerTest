    ...
    data = {
      ...
      nodeTitle: function (setting, node) {
        ...
        if ( node.mappingError ) {
          title = 'CSF：'
          if ( node.mappingList.length >= 0 ) {
            title += node.mappingList.join(' | ')
          } else {
            title += node.join('')
          }
        }
    ...
    view = {
      addNodes: function (setting, parentNode, index, newNodes, isSilent) {
        ...
        if ( parentNode ) {
          ...
          view.createNodes(setting, parentNode.level, newNodes, parentNode, index);
          if ( !isSilent ) {
      ...
      makeDOMNodeIcon: function (html, setting, node) {
        ...
        html.push("<span id='", node.tId, consts.id.ICON,
          ...
          "' class='", (consts.className.NAME + ' ' + className),
          "'>", name, "</span>");
      },
      ...
      makeNodeLineClass: function (setting, node) {
        ...
        if ( setting.view.showLine ) {
          if ( node.level == 1 && node.isFirstNode && node.isLastNode ) {
            lineClass.push(consts.line.ROOT);
          } else if ( node.level == 1 && node.isFirstNode ) {
  ...
  var _consts = {
    ...
    _handler = {
      ...
      onMousedownNode: function (eventMouseDown, _node) {
        ...
        if ( eventMouseDown.button == 2 || !setting.edit.enable || (!setting.edit.drag.isCopy && !setting.edit.drag.existMove) ) return true;
          ...
          if ( root.dragFlag == 1 ) {
            ...
            if ( tmpTarget && tmpTarget.attr("id") != targetSetting.treeId ) {
              var tmpTargetNode = tmpTargetNodeId == null ? null : data.getNodeCache(targetSetting, tmpTargetNodeId),
                isCopy = ((event.ctrlKey || event.metaKey) && setting.edit.drag.existMove && setting.edit.drag.isCopy) || (!setting.edit.drag.existMove && setting.edit.drag.isCopy),
        ...
        function _docMouseUp(event) {
          ...
          var isCopy = ((event.ctrlKey || event.metaKey) && setting.edit.drag.existMove && setting.edit.drag.isCopy) || (!setting.edit.drag.existMove && setting.edit.drag.isCopy);
